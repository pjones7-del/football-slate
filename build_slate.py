...
def previous_lines(tpl):
    """{event id: (spread, total)} from the template's data block, for games that had a line last build."""
    out = {}
    m = re.search(r"/\*DATA-START\*/(.*?)/\*DATA-END\*/", tpl, re.S)
    if not m:
        return out
...
def main():
    ...
    prev_lines = previous_lines(tpl)   # last line seen for each game in the previous build; ESPN drops odds at kickoff
